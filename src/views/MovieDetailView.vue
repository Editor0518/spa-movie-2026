...
const generateAIReview = async() => {
    ...
    try {
        const promptMessage = `
        너는 200만 구독자의 영화 평론 유튜버야. 아래 영화 데이터를 기반으로 블로그 글처럼
        2~3문단 분량의 상세하고 흡입력 있는 추천평을 작성해줘(이모지 필수).
        제목: ${store.selectedMovie.title},
        장르: ${store.selectedMovie.genres.map(g => g.name).join(', ')},
        평점: ${store.selectedMovie.vote_average.toFixed(1)} / 10 점,
        시놉시스: ${store.selectedMovie.overview}`
        ;
        ...
        aiReview.value = "서버가 혼잡합니다. 잠시 후 다시 시도해주세요.";
    } finally {
            ...
            <div class="movie-hero-grid">
                ...
                <div class="info-zone">
                    ...
                    <div class="meta-dashboard">
                        <span class="badge rating">평점 {{ store.selectedMovie?.vote_average?.toFixed(1) }} / 10</span>
                        <span class="badge runtime">상영 {{ store.selectedMovie?.runtime }} 분</span>
                        <span class="badge release">개봉 {{ store.selectedMovie?.release_date }} 개봉</span>
                    </div>
...
.synopsis-text { font-size: 17px; line-height: 1.9; color: #dcdee1; text-align: justify; margin: 0; }
